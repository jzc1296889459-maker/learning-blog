---
title: '从零搭建 GPT-2：精简训练流水线'
description: '将第 2–7 章的必要代码重排成一条可执行流水线：从 GPT-2 预训练到分类与指令微调。'
...
这篇文章把 _Build a Large Language Model (From Scratch)_ 第 2–7 章的必要代码重新排列成一条完整主线。第 1 章只作为路线图，没有可执行 notebook；第 2–5 章从文本数据一路搭建并预训练一个教学用 GPT-2，随后从全新的官方检查点分出两个实际任务。

- 主干：第 1 章路线图 → 第 2 章文本数据 → 第 3 章注意力 → 第 4 章 `GPTModel` → 第 5 章预训练与检查点
- 分类分支：全新官方 GPT-2 124M → 第 6 章短信分类微调
- 指令分支：全新官方 GPT-2 355M → 第 7 章指令微调

这里保留 notebook 中的全部代码，但不展开逐行解释。相关代码块后展示了精选输出；完整执行结果保留在[可下载 notebook](/notebooks/gpt2-from-scratch-training-pipeline.ipynb)中。

## 对应的详细笔记

- [第 2 章：文本数据](/notes/20260817---llm-from-scratch-ch02-text-data)
- [第 3 章：注意力](/notes/20260817---llm-from-scratch-ch03-attention)
- [第 4 章：GPT 模型](/notes/20260817---llm-from-scratch-ch04-gpt-model)
- [第 5 章：预训练与权重加载](/notes/20260820---llm-from-scratch-ch05-pretraining)
- [第 6 章：分类微调](/notes/20260820---llm-from-scratch-ch06-classification)
- [第 7 章：指令微调](/notes/20260822---llm-from-scratch-ch07-instruction-finetuning)
...
完整运行会在 `gpt2-pipeline-run/artifacts/` 中生成以下临时产物：
...
这些权重、数据集和运行目录不随博客发布；notebook 只保留验证流水线所需的精简输出。

## 来源与许可

本流水线选取并重排 Sebastian Raschka 的 [_Build a Large Language Model (From Scratch)_](https://www.manning.com/books/build-a-large-language-model-from-scratch) 及其[官方代码仓库](https://github.com/rasbt/LLMs-from-scratch)第 2–7 章的代码。原始代码按 [Apache License 2.0](https://github.com/rasbt/LLMs-from-scratch/blob/main/LICENSE.txt) 发布。这里仅做顺序整理、必要的连接修正与执行防护，不替代原书及其完整讲解。
